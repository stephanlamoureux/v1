YUI().use('node', 'node-event-simulate', function (Y) {
  var AniJSTest = {
      Utils: {
        settingEnviroment: function (dataAnijJS) {
          Y.one('#testzone').appendChild(
            "<div class='ancestorsTest red-ancestors'> " +
              "<div id='dv1' > </div> " +
  ...
  describe('Execute the selector fuction ancestor', function () {
    ...
    describe('on do action, without params. ', function () {
      beforeEach(function (done) {
        var dataAnijJS =
          'if: click, do: $addClass hidden, to: $ancestors target | .red-ancestors, after: $afterFunction'
    ...
    describe('on do action, without params. ', function () {
      beforeEach(function (done) {
        var dataAnijJS =
          'if: click, do: $addClass hidden, to: $ancestors .red-ancestors, after: $afterFunction'
        AniJSTest.Utils.settingHelper(dataAnijJS, done, 3)
      })
